...
contract TestOnlyPluginUpgraderCondition is OnlyPluginUpgraderCondition {
  ...
  function decodeGrantRevokeCalldata(bytes memory _data)
    public
    pure
  ...
  function decodeApplyUpdateCalldata(bytes memory _data)
    public
    pure
  ...
  function isValidGrantRevokeCalldata(bytes memory _grantData, bytes memory _revokeData) public view returns (bool) {
