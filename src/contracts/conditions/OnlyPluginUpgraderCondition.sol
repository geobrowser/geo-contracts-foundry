...
contract OnlyPluginUpgraderCondition is PermissionCondition {
  ...
  constructor(DAO _dao, PluginSetupProcessor _psp, address[] memory _targetPluginAddresses) {
    if (address(_dao) == address(0) || address(_psp) == address(0) || _targetPluginAddresses.length == 0) {
  ...
  function _decodeGrantRevokeCalldata(bytes memory _data)
    internal
    pure
    returns (bytes4 selector, address where, address who, bytes32 permissionId)
  {
  ...
  function _decodeApplyUpdateCalldata(bytes memory _data)
    internal
    pure
    returns (bytes4 selector, address daoAddress, address targetPluginAddress)
  {
    // Slicing is only supported for bytes calldata, not bytes memory
    // Bytes memory requires an assembly block
    assembly {
  ...
  function _isValidGrantRevokeCalldata(bytes memory _grantData, bytes memory _revokeData) internal view returns (bool) {
    // Grant checks
    (bytes4 _grantSelector, address _grantWhere, address _grantWho, bytes32 _grantPermission) =
    ...
    // Revoke checks
    (bytes4 _revokeSelector, address _revokeWhere, address _revokeWho, bytes32 _revokePermission) =
  ...
  function _isValidApplyUpdateCalldata(bytes memory _data) internal view returns (bool) {
    (bytes4 _selector, address _dao, address _targetPluginAddress) = _decodeApplyUpdateCalldata(_data);
