...
bytes4 constant SPACE_INTERFACE_ID = SpacePlugin.initialize.selector ^ SpacePlugin.publishEdits.selector
  ^ SpacePlugin.flagContent.selector ^ SpacePlugin.acceptSubspace.selector ^ SpacePlugin.removeSubspace.selector;
...
contract SpacePlugin is PluginUUPSUpgradeable {
  ...
  function supportsInterface(bytes4 _interfaceId) public view override(PluginUUPSUpgradeable) returns (bool) {
