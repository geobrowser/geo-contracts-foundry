...
- **`ping` and follow-on `Action` emissions** — **`DAOSpace.ping`** (only the DAO contract itself may call it: **`onlyRole(DAO)`** requires **`msg.sender == address(this)`**) invokes **`_ping`**, which calls **`SpaceRegistry.enter`** with both **`from`** and **`to`** set to the DAO’s space id so the registry emits further **`Action`** events—covering kinds that supplement direct user **`enter`** flows, including behaviour historically exposed via dedicated registry entrypoints.
...
**`spaceIdToProposedAddress`** is set by **`proposeSpaceMigration`** and cleared by **`acceptSpaceMigration`**, **`clearSpaceId`**, or owner **`overrideSpaceId`**. **`overrideSpaceId`** also sets **`archivedSpaceIds[_spaceId]`** to **false** for the **`spaceId`** being bound, so the id is active after rebinding (unless later archived again through the normal path). **Operators** should assume an owner rebinding cancels any in-flight migration proposal for that **`spaceId`**.
